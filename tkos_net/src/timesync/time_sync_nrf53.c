...
#define SHARED_MEMORY_BASE_ADDRESS DT_REG_ADDR(DT_NODELABEL(sram1_shared))
volatile ipc_sync_packet_t *shared_mem = (struct ipc_sync_packet_t *)SHARED_MEMORY_BASE_ADDRESS;
...
enum dppi_channels {
	...
	DPPI_CHN_MASTER_SYNCH_NODE, /* recieve IPC event to become master clock node */
	DPPI_CHN_TIMESTAMP_TRIGGER, /* recieve IPC event to capture timestamp */
	DPPI_CHN_COUNT,
};
...
static int32_t count_delta = 0;
static int64_t ticks_per_offset_avg = 0;
...
static mpsl_timeslot_request_t m_timeslot_req_earliest = {
		...
		.params.earliest.timeout_us = 1000000 /* Expect timeslot within 100 ms */
};
...
ISR_DIRECT_DECLARE(egu_isr)
{
	...
	/* This is the event triggered by the app core when master synch node is 
	 to be toggled on or off. */
	if (nrf_egu_event_check(EGU_INST, NRF_EGU_EVENT_TRIGGERED6))
	{
		nrf_egu_event_clear(EGU_INST, NRF_EGU_EVENT_TRIGGERED6);

		LOG_DBG("EGU event triggered 6; Toggling master synch node.");
		toggle_master_synch_node();
	}

	if (nrf_egu_event_check(EGU_INST, NRF_EGU_EVENT_TRIGGERED8)) 
	{
		nrf_egu_event_clear(EGU_INST, NRF_EGU_EVENT_TRIGGERED8);

		// get the timestamp from the counter timer (cc_channel2) and the freewheel timer (cc_channel4)
		uint32_t counter_val = nrf_timer_cc_get(COUNTER_TIMER, NRF_TIMER_CC_CHANNEL2) - count_delta;
		uint32_t timer_val = nrf_timer_cc_get(FREEWHEEL_TIMER, NRF_TIMER_CC_CHANNEL4);
		uint64_t timestamp = ((uint64_t)counter_val * TIME_SYNC_TIMER_MAX_VAL) + timer_val;
		int32_t drift_adjust_ticks = 0;

		// ensure that ticks_per_offset_avg is not zero before calculating drift_adj
		if (!(ticks_per_offset_avg == 0))
		{
			// adjust based on expected drift and time since last sync packet
			drift_adjust_ticks = ((int64_t)(timestamp - last_sync_timestamp)) / ticks_per_offset_avg;
		}

		LOG_DBG("EGU event triggered 8; Triggering timestamp capture logic.");
		LOG_DBG("count_delta: %d", count_delta);
		LOG_DBG("Counter Timer: %u", counter_val);
		LOG_DBG("Freewheel Timer: %u", timer_val);
		LOG_DBG("drift_adjust_ticks %d ticks", drift_adjust_ticks);
		LOG_DBG("time since last sync packet: %llu ticks", timestamp - last_sync_timestamp);
		LOG_DBG("Timestamp: %llu usec", timestamp/(uint64_t)16);
		LOG_DBG("Timestamp adjusted: %llu usec", ((int64_t)timestamp - drift_adjust_ticks)/(uint64_t)16);
	}
...
void timeslot_begin_handler(void)
{
	...
	// update the shared memory so the app core can synch with this net core
	shared_mem->timer_offset_ticks = 0;
	shared_mem->delta_time_ticks = 0;
	shared_mem->current_timestamp_ticks = (uint64_t)p_pkt->counter_val * (uint64_t)TIME_SYNC_TIMER_MAX_VAL + (uint64_t)p_pkt->timer_val;
	shared_mem->peer_counter = p_pkt->counter_val;
	// trigger the ipc event to notify the app core that the shared memory has been updated
	nrf_ipc_task_trigger(NRF_IPC, NRF_IPC_TASK_SEND_7);
}
...
static inline bool sync_timer_offset_compensate(struct sync_pkt *p_pkt)
{
	...
	uint64_t peer_current_timestamp;
	uint64_t local_current_timestamp;
    bool wrapped = false;
    ...
    // Calculate peer timer adjusted for transmission delay
    peer_timer = p_pkt->timer_val + TX_CHAIN_DELAY;
	...
	// Calculate peer timer adjusted for transmission delay w/ counter 
	peer_current_timestamp = (uint64_t)p_pkt->timer_val + (uint64_t)TX_CHAIN_DELAY +
                         (uint64_t)p_pkt->counter_val * (uint64_t)TIME_SYNC_TIMER_MAX_VAL;


    // Read the local timer value
    local_timer = nrf_timer_cc_get(FREEWHEEL_TIMER, NRF_TIMER_CC_CHANNEL7);
	local_counter = nrf_timer_cc_get(COUNTER_TIMER, NRF_TIMER_CC_CHANNEL7);
    timer_offset = local_timer - peer_timer;  // Initial drift calculation in microseconds
	...
	// log the difference in the COUNTER value
	count_delta = ((int32_t)nrf_timer_cc_get(COUNTER_TIMER, NRF_TIMER_CC_CHANNEL7)) - p_pkt->counter_val;
	LOG_DBG("COUNT difference: %d", count_delta);
	

    // Get the current timestamp in ticks
    local_current_timestamp = ((uint64_t)nrf_timer_cc_get(COUNTER_TIMER, NRF_TIMER_CC_CHANNEL7) - count_delta) * TIME_SYNC_TIMER_MAX_VAL + local_timer;

    // Calculate the time delta since the last sync in microseconds (16 MHz timer)
    uint32_t delta_time_ticks = (local_current_timestamp - last_sync_timestamp);

    // Avoid division by zero if this is the first sync
    if (last_sync_timestamp != 0 && delta_time_ticks > 0) {
        // Store the timer offset in ticks and the total elapsed time in ticks 
        shared_mem->current_timestamp_ticks = peer_current_timestamp;
		shared_mem->timer_offset_ticks = timer_offset;
		shared_mem->delta_time_ticks = delta_time_ticks;
		shared_mem->peer_counter = p_pkt->counter_val;

		// might need to use the nrf_ipc_task_address_get to get the specific register address
        // Trigger the IPC task for the application core to read the drift rate
        nrf_ipc_task_trigger(NRF_IPC, NRF_IPC_TASK_SEND_7);
    }

    // Update the last sync timestamp
    last_sync_timestamp = local_current_timestamp;
    ...
    if (shorten_cycle) {
        // Clear the timer earlier than normal and increment the counter
        nrf_timer_cc_set(FREEWHEEL_TIMER, NRF_TIMER_CC_CHANNEL1, TIME_SYNC_TIMER_MAX_VAL + timer_offset);
    } else {
        // Clear timer right after normal clear, no counter increment
        nrf_timer_cc_set(FREEWHEEL_TIMER, NRF_TIMER_CC_CHANNEL2, timer_offset);
    }

	// calculate the number of ticks it takes for one tick offset to occur
	int32_t ticks_per_offset = (int32_t)delta_time_ticks / timer_offset;
	// estimate a moving average for the ticks per offset
	static int32_t sample_count = 0;
	#define AVG_WINDOW_SIZE 20
	if (sample_count < AVG_WINDOW_SIZE) {
		ticks_per_offset_avg = ((ticks_per_offset_avg * sample_count) + ticks_per_offset) / (sample_count + 1);
		sample_count++;
	} else {
		ticks_per_offset_avg = (ticks_per_offset_avg * AVG_WINDOW_SIZE + ticks_per_offset) / (AVG_WINDOW_SIZE + 1);
	}

	// estimate the drift over 10 seconds
	int32_t expected_drift = (10 * 1e6 * 16) / (int32_t)(ticks_per_offset_avg); // in microseconds

    atomic_clear(&m_timer_update_in_progress);

	LOG_INF("ticks_per_offset: %d", ticks_per_offset);
	LOG_INF("ticks_per_offset_avg: %d", ticks_per_offset_avg);
	LOG_INF("expected drift in ticks over 10 sec: %d", expected_drift);
	LOG_INF("timer_offset_ticks: %d", timer_offset);
	LOG_INF("local_timer: %u", local_timer);
	LOG_INF("peer_timer: %u", peer_timer);
	LOG_INF("current_timestamp_ticks: %llu", local_current_timestamp);
	LOG_INF("peer current_timestamp_ticks: %llu", peer_current_timestamp);
	LOG_INF("delta_time_ticks: %u", delta_time_ticks);
	LOG_INF("Net core local freewheel timer: %u", local_timer);
	LOG_INF("Net core local counter timer: %u", local_counter);
	LOG_INF("---");
...
static void dppi_radio_rx_configure(void)
{
	...
	// configure ipc to trigger on the same event so the app core captures timer values at the same time
	nrf_ipc_subscribe_set(
		NRF_IPC, NRF_IPC_TASK_SEND_5, m_dppi_channels[DPPI_CHN_CAPTURE_RADIO]);
...
int ts_init(ts_evt_handler_t evt_handler)
{
	...
	for (int i = 0; i < ARRAY_SIZE(m_dppi_channels); ++i) {
		// probably need to move this dppi instance to be global and utilize it throughout the application
		nrfx_err = nrfx_dppi_channel_alloc(&dppi_inst, &m_dppi_channels[i]);
		if (nrfx_err != NRFX_SUCCESS) {
			return -ENODEV;
		}
	}

	// set-up the master synch node egu and dppi
	nrf_ipc_receive_config_set(NRF_IPC, NRF_IPC_EVENT_RECEIVE_6, NRF_IPC_CHANNEL_6);
	// configure the IPC to publish the RECEIVE0 event on the DPPI channel
	nrf_ipc_publish_set(NRF_IPC, NRF_IPC_EVENT_RECEIVE_6, m_dppi_channels[DPPI_CHN_MASTER_SYNCH_NODE]);
	// trigger an interrupt when the event is received
	nrf_egu_subscribe_set(EGU_INST, NRF_EGU_TASK_TRIGGER6, m_dppi_channels[DPPI_CHN_MASTER_SYNCH_NODE]);
	// enable dppi channel for master synch node ipc event
	nrf_dppi_channels_enable(NRF_DPPIC_NS, BIT(m_dppi_channels[DPPI_CHN_MASTER_SYNCH_NODE]));

	// set-up timestamp capture ipc and dppi
	nrf_ipc_receive_config_set(NRF_IPC, NRF_IPC_EVENT_RECEIVE_8, NRF_IPC_CHANNEL_8);
	nrf_ipc_publish_set(NRF_IPC, NRF_IPC_EVENT_RECEIVE_8, m_dppi_channels[DPPI_CHN_TIMESTAMP_TRIGGER]);
	nrf_egu_subscribe_set(EGU_INST, NRF_EGU_TASK_TRIGGER8, m_dppi_channels[DPPI_CHN_TIMESTAMP_TRIGGER]);
	nrf_dppi_channels_enable(NRF_DPPIC_NS, BIT(m_dppi_channels[DPPI_CHN_TIMESTAMP_TRIGGER]));
	// set-up the timers to capture their timestamp on the same dppi channel
	nrf_timer_subscribe_set(FREEWHEEL_TIMER, NRF_TIMER_TASK_CAPTURE4, m_dppi_channels[DPPI_CHN_TIMESTAMP_TRIGGER]);
	nrf_timer_subscribe_set(COUNTER_TIMER, NRF_TIMER_TASK_CAPTURE2, m_dppi_channels[DPPI_CHN_TIMESTAMP_TRIGGER]);
	...
	// enable ipc channel 7 for drift rate communication
	nrf_ipc_send_config_set(NRF_IPC, NRF_IPC_TASK_SEND_7, NRF_IPC_CHANNEL_7);
	nrf_ipc_send_config_set(NRF_IPC, NRF_IPC_TASK_SEND_5, NRF_IPC_CHANNEL_5);
...
int ts_tx_start(uint32_t sync_freq_hz)
{
	...
	// clear the timers so they start from 0
	nrf_timer_task_trigger(FREEWHEEL_TIMER, NRF_TIMER_TASK_CLEAR);
	nrf_timer_task_trigger(COUNTER_TIMER, NRF_TIMER_TASK_CLEAR);
